...
export interface StepInput {
  ...
  ragEnabled?: boolean; // шаг сверяется с базой знаний перед тем, как считать цель достигнутой
}
...
// Статическая инструкция для RAG-шага — сама выдержка из базы подставляется во время
// диалога движком (MessageHandler), под последнее сообщение клиента; тут только правило,
// как её использовать при решении completed: true/false.
function buildRagBlock(ragEnabled: boolean | undefined): string {
  if (!ragEnabled) return "";
  return `[ПРОВЕРКА ПО БАЗЕ ЗНАНИЙ]
Под этой инструкцией может быть блок «КОНТЕКСТ ИЗ БАЗЫ ЗНАНИЙ» — выдержка из базы бота (например, каталог/прайс, где у одной позиции бывает несколько вариантов, различающихся какой-то характеристикой — со своим артикулом/ценой/наличием у каждого), отобранная под последнее сообщение клиента. Поиск по базе приблизительный (по смыслу) и может подсунуть ПОХОЖИЙ, но другой код/артикул/название — сверяй их БУКВАЛЬНО: если хоть одна цифра/буква в номере/названии отличается от того, что назвал клиент — считай, что именно этого в контексте НЕТ, даже если рядом есть очень похожий вариант.
- ТОЧНОЕ совпадение (в контексте однозначно есть РОВНО ОДНА строка/вариант, отвечающая ВСЕМ характеристикам, которые уже назвал клиент) — веди себя как обычно: считай цель шага достигнутой (completed: true), сохрани значение.
- ЧАСТИЧНОЕ совпадение (в контексте ещё НЕСКОЛЬКО строк/вариантов подходят под сказанное клиентом, различаются каким-то ещё не названным клиентом признаком, от которого зависят наличие/цена/артикул) — НЕ считай цель выполненной (completed: false) и НЕ переходи дальше по сценарию; задай ОДИН точечный вопрос именно про этот оставшийся неясный признак (назови его так, как он обозначен в контексте), пока не останется ровно одна подходящая строка. Не выдумывай и не предполагай наличие/цену для варианта, который ещё не сужен до одной строки.
- НЕТ совпадения (в контексте явно НЕТ того, что назвал клиент — включая случай, когда есть только похожий, но не совпадающий буквально вариант) — НЕ считай цель выполненной (completed: false); вежливо и по-человечески сообщи, что этого нет, и предложи уточнить или назвать другой вариант. Не выдумывай то, чего нет в контексте.
- Клиент ОТКАЗАЛСЯ от предложенной альтернативы (в ПРЕДЫДУЩЕЙ своей реплике ты уже сообщил, что нужного нет, и спросил, можешь ли помочь с чем-то другим, а клиент отвечает отказом — «нет», «не», «ничего», «не надо», «нет, спасибо») — то, что клиент хотел, дать нельзя, и другое ему не интересно: НЕ начинай сценарий заново и НЕ переспрашивай цель обращения с нуля. Заверши диалог одной тёплой прощальной фразой (без канцелярита) и верни "abort": true.
- Если блока с контекстом нет вовсе (база ничего не нашла по запросу) — веди себя как обычно, без завязки на базу.`;
}

// RAG-проверка для router/routedStep: у этого контракта нет поля "completed" (только
// next_process), поэтому обычный buildRagBlock тут не подходит — реши next_process, а не
// просто заяви цель достигнутой. Три исхода: точное совпадение / нет совпадения / частичное.
function buildRagRoutingBlock(ragEnabled: boolean | undefined): string {
  if (!ragEnabled) return "";
  return `[ПРОВЕРКА ПО БАЗЕ ЗНАНИЙ]
Под этой инструкцией может быть блок «КОНТЕКСТ ИЗ БАЗЫ ЗНАНИЙ» — выдержка из базы бота, отобранная под последнее сообщение клиента. Она влияет на next_process — не выбирай ветку только по факту, что клиент что-то назвал, сверяйся с контекстом. Поиск по базе приблизительный (по смыслу) и может подсунуть ПОХОЖИЙ, но другой код/артикул/модель — сверяй их БУКВАЛЬНО: если хоть одна цифра/буква в номере/модели отличается от того, что назвал клиент — считай, что именно этого в контексте НЕТ, даже если рядом есть очень похожий вариант.
- ТОЧНОЕ совпадение (в контексте БУКВАЛЬНО есть именно то, что назвал клиент) — товар нашёлся, это сильный сигнал к покупке: спроси ПРЯМО про покупку («значит, интересует <точное название из контекста> — хотите его купить/оформить?»), next_process=null. В ЭТОМ случае НЕ задавай общий вопрос со списком всех веток из [ВЕТВЛЕНИЕ] ниже — раз товар точно нашёлся, уточняющий вопрос именно про покупку, а не про абстрактную «цель обращения». Клиент подтвердил → next_process = ветка, отвечающая за покупку/заказ; если клиент уточнил, что это была консультация или просто вопрос о наличии — next_process = соответствующая другая ветка.
- НЕТ совпадения (в контексте явно НЕТ того, что назвал клиент — включая случай, когда есть только похожий, но не совпадающий буквально код) — вежливо скажи, что такого нет, предложи назвать другой вариант; next_process=null. Не выдумывай то, чего нет в контексте.
- ЧАСТИЧНОЕ совпадение (в контексте есть несколько похожих/смежных вариантов, но не точно то, что назвал клиент) — перечисли их и спроси, какой нужен; next_process=null, пока клиент не выберет один.
- Если блока с контекстом нет вовсе (база ничего не нашла) — реши next_process по условиям ветвления ниже как обычно, без завязки на базу.`;
}
...
ВАЖНО: приветствие и статус сущности этого шага — НЕЗАВИСИМЫ. Даже если это самое первое сообщение клиента за весь диалог (greeted=false, история пустая) — если клиент В ЭТОМ ЖЕ сообщении уже дал ответ по цели шага, обработай его как обычно (статус A/B/D по [СТАТУС СУЩНОСТИ]), НЕ притворяйся, что ответа не было, и НЕ переспрашивай то, что он уже сказал. Приветствие в этом случае — это только ПЕРВАЯ фраза bot_message, а дальше — обычная обработка ответа в том же сообщении.`;
}
...
A) ПРИНЯТА — сущность теперь известна ПОЛНОСТЬЮ: клиент в текущем сообщении назвал её целиком, подтвердил («да», «верно»), исправил на другую, ИЛИ этим сообщением ответил на твой же уточняющий вопрос (см. D) → known_updates["${key}"] = ПОЛНОЕ значение. Если текущее сообщение — только уточнение/довесок к тому, что клиент называл раньше в этой же теме (например отвечает одним словом на твой уточняющий вопрос) — СКЛЕЙ его с уже названным раньше в один цельный ответ, не сохраняй в поле только последнее слово отдельно. current_task_completed=true, bot_message="".
...
D) НЕОДНОЗНАЧНО — значение неполное/неясное по правилу, или в сообщении путаница → current_task_completed=false, точечный уточняющий вопрос. НЕ записывай в known_updates частичное/неполное значение — дождись ответа клиента и тогда сохрани ПОЛНОЕ склеенное значение по правилу A. (Ясно названное значение — это A, а не D.)${opt}`;
}
...
function assembleStepPrompt(params: {
  ...
  const { kind, companyName, goal, style, required, rules, branches, body, fieldName, slots = [], nonGoals = [], scenarioGoals = [], ragEnabled } = params;
  ...
  if (canExit) parts.push(buildRagRoutingBlock(ragEnabled));
  else if (kind !== "completion") parts.push(buildRagBlock(ragEnabled));
  ...
  parts.push(uFormat(kind, (!!oos || !!ragEnabled) && kind !== "completion"));
