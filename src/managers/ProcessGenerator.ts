...
Под этой инструкцией может быть блок «КОНТЕКСТ ИЗ БАЗЫ ЗНАНИЙ» — выдержка из базы бота, отобранная под последнее сообщение клиента.
- Если контекст есть и в нём явно ЕСТЬ то, что упомянул клиент — веди себя как обычно: считай цель шага достигнутой (completed: true), сохрани значение.
- Если контекст есть, но в нём явно НЕТ того, что упомянул клиент (например, конкретной модели/товара/услуги) — НЕ считай цель выполненной (completed: false); вежливо и по-человечески сообщи, что этого нет, и предложи уточнить или назвать другой вариант. Не выдумывай то, чего нет в контексте.
...
function assembleStepPrompt(params: {
  ...
  if (kind !== "completion") parts.push(buildRagBlock(ragEnabled));
